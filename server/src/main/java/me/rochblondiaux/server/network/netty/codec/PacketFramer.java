            ...
            if (b >= 0) {
                ...
                if (packetSize >= 30000) {
                    ...
                    if (clientConnection != null)
                        log.warn("An user ({}) sent a packet over the maximum size ({})", clientConnection.getRemoteAddress(), packetSize);
                    else
                        log.warn("An unregistered user sent a packet over the maximum size ({})", packetSize);
                    ctx.close();
                }
